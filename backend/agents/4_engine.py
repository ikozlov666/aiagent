    ...
    async def _run_loop(
        ...
        for iteration in range(max_iterations):
            ...
            async def execute_single_tool(tool_call, step_offset):
                ...
                # Safe self-healing for write_files (NO protocol violation)
                if tool_name == "write_files":
                    if not isinstance(tool_args, dict) or not tool_args.get("files"):
                        print("🚨 [Agent] write_files called without valid 'files' — returning structured error")
                        tool_args = {"__invalid_write_files__": True}
                ...
                # Execute
                try:
                    if tool_name == "write_files" and "__invalid_write_files__" in tool_args:
                        result = {
                            "success": False,
                            "error": "write_files requires a non-empty 'files' array"
                        }
                    else:
