...
class AgentEngine:
    ...
    def _validate_messages(messages: list[dict]) -> list[dict]:
        """
        Safety net: ensure every 'tool' message is preceded by an 'assistant'
        with matching tool_calls.  Drop orphaned tool messages to prevent
        API errors.
        """
    ...
    def _get_messages_for_llm(self) -> list[dict]:
        ...
        out.extend(recent)
        return self._validate_messages(out)
