...
class AgentEngine:
    ...
    def _normalize_tool_arguments(self, tool_name: str, raw_args):
        """Robust tool argument normalizer."""
        if raw_args is None:
            return {}
        ...
        if isinstance(raw_args, str):
            ...
            if not raw_args or raw_args.lower() == "null":
                return {}
            try:
                return json.loads(raw_args)
            except Exception:
                print(f"⚠️ [Agent] Failed to parse JSON for {tool_name}, attempting repair...")
                return self._repair_json_args(tool_name, raw_args)
    ...
    async def _run_loop(
        ...
        for iteration in range(max_iterations):
            ...
            async def execute_single_tool(tool_call, step_offset):
                ...
                try:
                    tool_args = self._normalize_tool_arguments(tool_name, raw_args)
                except Exception as e:
                    print(f"❌ [Agent] Unexpected error parsing arguments for {tool_name}: {e}")
                    tool_args = {}

                # Safety guard for write_files
                if tool_name == "write_files":
                    if not isinstance(tool_args, dict) or not tool_args.get("files"):
                        print("🚨 [Agent] write_files called without valid 'files' argument")
                        tool_args = {"files": []}
