...
class AgentEngine:
    ...
    def __init__(self, project_id: str, use_multi_agent: bool = False):
        ...
        self._same_tool_counter = 0
        self._empty_tool_iterations = 0
    ...
    def _get_messages_for_llm(self) -> list[dict]:
        ...
        # Split: old (to summarize) + recent (keep full)
        n_full = min(LAST_N_MESSAGES_FULL, len(rest))
        old_part = rest[:-n_full]
        recent = rest[-n_full:]
    ...
    async def run(
        ...
        if self._task_class == "simple_chat":
            try:
                ...
                print(f"⚠️ [Agent] Fast path failed, falling back to full loop: {e}")
        ...
        if should_use_parallel_plan(user_message, images):
            try:
                ...
                print(f"⚠️ [Agent] Parallel plan failed, falling back to single agent: {e}")
    ...
    async def _run_loop(
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                # Determine provider: use escalated provider or default
                effective_task_type = task_type
                if self.escalation.escalated:
                    # Override routing to use the escalated provider directly
                ...
                if (not self.escalation.escalated
                        ...
                        and max_tokens_loop > 1536):
                    max_tokens_loop = 1536

                print(f"📡 [Agent] Отправляю запрос к LLM (task: {effective_task_type}, "
            ...
            # Track empty tool iterations
            if not message.tool_calls:
                self._empty_tool_iterations += 1
                if self._empty_tool_iterations >= 3:
                    print("🚨 [Agent] Repeated empty responses — escalating")
                    self.escalation.mark_escalated(self.escalation.current_provider)
            else:
                self._empty_tool_iterations = 0
            ...
            # Limit excessive tool calls from LLM
            if message.tool_calls and len(message.tool_calls) > 10:
                print("🚨 [Agent] Too many tool calls in one iteration — limiting to 10")
                message.tool_calls = message.tool_calls[:10]
            # ── Process tool calls ────────────────────────────────
            ...
            async def execute_single_tool(tool_call, step_offset):
                ...
                async with semaphore:
                    ...
                    try:
                        current_hash = make_args_hash(tool_call.function.arguments or "")
                    except Exception:
                        current_hash = None

                    if self._last_tool_call_hash == (tool_name, current_hash):
                        self._same_tool_counter += 1
                    else:
                        self._same_tool_counter = 0

                    self._last_tool_call_hash = (tool_name, current_hash)

                    if self._same_tool_counter >= 2:
                        print("🚨 [Agent] Detected repeated identical tool calls — escalating")
                        self.escalation.mark_escalated(self.escalation.current_provider)
                    ...
                    try:
                        ...
                        print(f"❌ [Agent] Unexpected error parsing arguments for {tool_name}: {e}")
                        tool_args = {}
                    ...
                    try:
                        ...
                        print(f"❌ [Agent] Ошибка выполнения {tool_name}: {e}")
