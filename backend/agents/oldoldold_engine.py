    ...
    async def _run_loop(
        ...
        for iteration in range(max_iterations):
            ...
            async def execute_single_tool(tool_call, step_offset):
                ...
                try:
                    if raw_args:
                        try:
                            ...
                            tool_args = self._repair_json_args(tool_name, raw_args)
                    else:
                        tool_args = {}
                except Exception as e:
                ...
                # Self-healing guard for write_files
                if tool_name == "write_files":
                    if not isinstance(tool_args, dict) or not tool_args.get("files"):
                        print("🚨 [Agent] write_files called without valid 'files' — asking model to fix")

                        # Return structured tool error to LLM instead of calling executor
                        self.messages.append({
                            "role": "tool",
                            "tool_call_id": tool_call.id,
                            "content": "ERROR: write_files requires a non-empty 'files' array. Provide valid file objects."
                        })
                        ...
                        return {
                            ...
                            "result": {
                                "success": False,
                                "error": "write_files called without files"
                            }
                        }
